...
import { VerifiedAddressSignature } from './verifiable-paystring'
...
export async function generateNewKey(): Promise<JWK> {
  ...
  return fromKeyLike(privateKey).then(async (key) => {
    return calculateThumbprint(key).then((thumbprint) => {
      return {
        kid: thumbprint,
